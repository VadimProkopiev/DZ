...
double FindCorner()
{
    double alfa = Math.Acos(
        Math.Abs((Math.Pow(y, 2) + Math.Pow(z, 2) - Math.Pow(x, 2)) / (2 * z * y))
    );
    double alfaDeg = alfa * 180 / Math.PI;
    alfaDeg = Math.Round(alfaDeg, 2);
    double beta = Math.Acos(
        Math.Abs((Math.Pow(x, 2) + Math.Pow(z, 2) - Math.Pow(y, 2)) / (2 * z * x))
    );
    ...
    if (alfaDeg == 90 || betaDeg == 90 || gamaDeg == 90)
        Console.WriteLine("Такой треугольник является прямоугольным");
    if (x == y && y == z && z == x)
        Console.WriteLine("Такой треугольник является равносторонним");
    if (
        alfaDeg == betaDeg && betaDeg != gamaDeg
        || betaDeg == gamaDeg && gamaDeg != alfaDeg
        || gamaDeg == alfaDeg && betaDeg != alfaDeg
    )
        Console.WriteLine("Такой треугольник является равнобедренным");
    return alfa;
}
void FindParametrs()
{
